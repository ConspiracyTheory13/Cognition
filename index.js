...
// reloadQuiz has to render a restart button that reloads
  ...
  if (submitBttn) submitBttn.addEventListener('click', renderQuizFeedback);
  if (reloadBttn) reloadBttn.addEventListener('click', reloadQuiz);

  const reloadQuiz = () => {
    let reloadBttn =`<button type="button" id="reloadBttn" class="reloadBttn"
